...
use vm::{AnyError, LoadedScriptsStore};
...
pub fn console_log(state: &mut OpState, args: LogMessage, _: ()) -> Result<(), AnyError> {
    let script_store = state.borrow::<LoadedScriptsStore>();
    ...
    let (name, line_col) = if let (Some(orig_name), Some(line)) = (args.file_name, args.line_number)
    {
        ...
        if let Some((src_name, src_line, src_col)) =
            script_store.get_original_line_col(&orig_name, line, col)
        {
